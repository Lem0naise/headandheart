...
export const searchMedia = action({
  ...
  handler: async (_ctx, args): Promise<{ title: string; year?: string; poster?: string; type: "movie" | "book" | "tvshow" | "videogame" | "boardgame" }[]> => {
    const q = encodeURIComponent(args.query.trim());
    if (!q || q.length < 2) return [];
    const results: { title: string; year?: string; poster?: string; type: "movie" | "book" | "tvshow" | "videogame" | "boardgame" }[] = [];
        ...
        if (!res.ok) {
          ...
          if (data.query?.search) {
            for (const r of data.query.search) {
              results.push({ title: r.title, type: args.type });
            }
          }
        }
